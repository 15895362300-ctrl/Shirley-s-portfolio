(()=>{
 ...
 const urls=[base+'inches-agent/',base+'mini-atelier.html',base+'paint-app/',base+'#project-4',base+'jd-campaign.html',base+'content-operations.html',base+'auction/',base+'#project-8'];
 ...
 const go=i=>{if(isModal&&[3,7].includes(i)){window.openProject(i);return}location.href=urls[i]};
 const back=document.createElement('button');back.type='button';back.className='pn-back';back.textContent='← 返回上一页';back.onclick=()=>{if(isModal){document.querySelector('#closeProject').click();return}if(history.length>1&&document.referrer&&new URL(document.referrer).origin===location.origin){history.back()}else location.href=base+'#projects'};
 ...
 const home=document.createElement('a');home.href=base+'#home';home.textContent='⌂  首页';panel.append(home);names.forEach((name,i)=>{const link=document.createElement('a');link.href=urls[i];link.textContent=(i+1)+'  '+name;if(i===index)link.setAttribute('aria-current','page');link.onclick=e=>{e.preventDefault();go(i)};panel.append(link)});
 ...
 const closing=isModal?null:document.querySelector('main .closing');if(closing){closing.insertAdjacentElement('afterend',bottom)}else root.append(bottom);
